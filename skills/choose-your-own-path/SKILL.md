...
```bash
npx -y magicpath-ai <command>
```

If auth is missing, run the MagicPath login flow or tell the user to log in, then verify again. Prefer `-o json` for data-returning commands. Discover the current CLI surface with `npx -y magicpath-ai --help` and command-specific help when creating or submitting new designs. Choose the CLI route that fits the installed version rather than assuming one fixed command exists.
